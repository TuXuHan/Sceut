...
請返回JSON格式：
{
  "analysis": "專業香調分析（嚴格控制在300-400字之間，並且要求分段，必須直接對用戶說話使用「您」而非「您好！」開頭，必須包含具體香調結構如前調中調基調、具體香料成分如佛手柑檀香等、專業使用技巧、季節環境建議、香水濃度建議，避免使用英文詞彙。請確保內容結構完整：香調分析（150字）+ 使用技巧（150字）+ 濃度建議（120字）+ 保存建議（100字）= 約520字）",
  "brands": [
    {
