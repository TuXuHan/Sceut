...
export async function GET(request: NextRequest) {
  try {
    ...
    if (fetchError) {
      console.error("❌ [CRON] 獲取訂閱記錄失敗:", fetchError)
      return NextResponse.json(
    ...
    if (!subscriptions || subscriptions.length === 0) {
      console.log("ℹ️ [CRON] 沒有需要處理的訂閱記錄")
      return NextResponse.json({
        success: true,
    ...
    console.log(`📊 [CRON] 找到 ${subscriptions.length} 筆訂閱記錄需要處理`)
    ...
    for (const subscription of subscriptions) {
      try {
        ...
        if (!createdAt || isNaN(createdAt.getTime())) {
          console.error(`❌ [CRON] 無效的 created_at: ${subscription.created_at}`)
          results.errors.push({
            id: subscription.id,
        ...
        if (updateError) {
          console.error(`❌ [CRON] 更新失敗 (訂閱 ${subscription.period_no || subscription.id}):`, updateError)
          results.errors.push({
            id: subscription.id,
            period_no: subscription.period_no || null,
        ...
        console.log(`✅ [CRON] 已更新訂閱 ${subscription.period_no || subscription.id}`)
        results.updated++
      } catch (error) {
        console.error(`❌ [CRON] 處理訂閱錯誤:`, error)
        results.errors.push({
          id: subscription.id,
          period_no: subscription.period_no || null,
    ...
    console.log("✅ [CRON] 定時重新計算完成!")
    console.log(
      `📊 [CRON] 統計: 總共 ${results.total} 筆, ` +
    ...
    console.error("❌ [CRON] 定時任務執行錯誤:", error)
    return NextResponse.json(
      {
        success: false,
        error: "[CRON] 定時任務執行失敗",
